...
local function dCheckPosMove(map,pos,ClosedList,OpenList,ClosedCount,Count)
	local Dat = {
		Right 	= true,
		Left 	= true,
		Up 		= true,
	...
	for k = 1,ClosedCount do
		...
		if (!Dat.Right and !Dat.Left and !Dat.Up and !Dat.Down) then return Dat end
	end
...
local function PerformAStar_CheckNodes(map,startpos,endpos,dFunc)
	...
	while (Count > 0 and RunTime < 9000) do
		...
		local Dat = dFunc(map,CurrentNode.Pos,ClosedList,OpenList,ClosedCount,Count)
